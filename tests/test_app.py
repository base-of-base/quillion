...
class TestQuillion:
    ...
    async def test_handler_key_exchange_failure(self, quillion, mock_websocket):
        ...
        with patch.object(
            quillion.crypto, "handle_key_exchange", AsyncMock(return_value=False)
        ):
            with patch("quillion_cli.debug.debugger.debugger") as mock_debugger:
                ...
                quillion.crypto.handle_key_exchange.assert_called_once_with(
    ...
    async def test_navigate_https_redirect(self, quillion, mock_websocket):
        ...
        with patch.object(
            quillion.crypto, "encrypt_response", return_value={"encrypted": "data"}
        ):
        ...
        with patch.object(quillion.asset_server, "start") as mock_asset_start:
            with patch.object(quillion.server_connection, "start") as mock_server_start:
                ...
                mock_server_start.assert_called_once_with(
                    quillion.handler, "127.0.0.1", 8080
                )
    ...
    def test_start_method_with_env_vars(self, quillion):
        with patch.dict(
            ...
            with patch.object(quillion.asset_server, "start") as mock_asset_start:
                with patch.object(
                    quillion.server_connection, "start"
                ) as mock_server_start:

                    quillion.start()

                    mock_asset_start.assert_called_once_with(
                        host="env_assets", port=8888
                    )
                    mock_server_start.assert_called_once_with(
                        quillion.handler, "env_host", 9999
                    )
