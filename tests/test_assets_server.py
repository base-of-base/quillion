...
class TestAssetServer:
    ...
    async def test_subdirectory_with_spaces(self, asset_server, temp_assets_dir):
        ...
        response = await self.make_request(
            asset_server, "/sub%20directory/file%20with%20spaces.txt"
        )
        if response.status == 200:
            content = await self.get_response_content(
                response, temp_assets_dir, "/sub directory/file with spaces.txt"
            )
            assert "content with spaces" in content
        else:
            response = await self.make_request(
                asset_server, "/sub directory/file with spaces.txt"
            )
            assert True
