...
class TestPageDecorator:
    ...
    def test_page_decorator_with_pattern_route(self):
        ...
        for registry in [
            PageMeta._registry,
            PageMeta._dynamic_routes,
            PageMeta._regex_routes,
        ]:
            for key in registry:
    ...
    def test_multiple_page_decorators(self):
        ...
        for registry in [
            ...
            for key, (page_class, priority) in (
                registry.items() if hasattr(registry, "items") else []
            ):
...
class TestPageDecoratorValidation:
    ...
    def test_page_decorator_with_type_hints(self):
        ...
        @page("/type-hints")
        def type_hints_page(
            items: List[str], count: int, optional: Optional[float] = None
        ):
...
class TestPageDecoratorPerformance:
    ...
    async def test_concurrent_async_pages(self):
        ...
        results = await asyncio.gather(page1_instance.render(), page2_instance.render())
