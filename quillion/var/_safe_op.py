...
from ._strategies import _OP_STRATEGIES, _as_float, _as_is, _as_str
...
def _safe_op(op_func: Callable[[Any, Any], Any], a: Any, b: Any) -> Any:
    ...
    try:
        ...
        if op_func in (operator.eq, operator.ne, operator.lt, operator.le, operator.gt, operator.ge) or op_func in (operator.add, operator.mul):
