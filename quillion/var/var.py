...
class Var(ReactiveExpression):
    ...
    _observers: WeakKeyDictionary[Component, list[Callable]]
    _raw_callbacks: list[Callable[[Any, Any], None]]
    ...
    def __init__(self, initial_value: Any = "") -> None:
        ...
        self._raw_callbacks: list[Callable[[Any, Any], None]] = []
    ...
    def value(self) -> Any:
        ...
        session: Session = ctx.current_session.get()
        if session is None:
            return self._initial

        raw_value: Any = session.state.get_var_value(self._key, None)

        if raw_value is None:
            new_value: Any = self._factory()
            wrapped: Any = self._wrap_value(new_value)
    ...
    def __call__(self, *args: Any, as_component: Any = None, **kwargs: Any) -> Any:
    ...
    def _add_raw_callback(self, cb: Callable[..., Any]) -> Callable[[], None]:
...
class _ReactiveVarTransformer(ast.NodeTransformer):
                ...
                if isinstance(node, ast.ImportFrom):
                    ...
                    for alias in node.names:
                        if alias.name == 'quillion':
                            return tree
        import_node: ast.ImportFrom = ast.ImportFrom(
            module='quillion',
    ...
    def visit_Assign(self, node: ast.Assign) -> Any:
        ...
        new_value: ast.Call = self._wrap_in_var(node.value)
        new_node: ast.Assign = ast.Assign(
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> Any:
        ...
        new_node: ast.AnnAssign = ast.AnnAssign(
            target=node.target,
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        for arg in node.args:
            if isinstance(arg, ast.Lambda):
                new_args.append(self.visit_Lambda(arg))
        ...
        for kw in node.keywords:
            if isinstance(kw.value, ast.Lambda):
                new_keywords.append(ast.keyword(arg=kw.arg, value=self.visit_Lambda(kw.value)))
    ...
    def visit_Lambda(self, node: ast.Lambda) -> Any:
        if isinstance(node.body, ast.BinOp):
            transformed: ast.AST | None = self._transform_binop(node.body)
    ...
    def _process_function_body(self, body: list[ast.stmt]) -> list[ast.stmt]:
        ...
        for stmt in body:
            if isinstance(stmt, ast.Assign):
                ...
                if isinstance(stmt.value, ast.Lambda):
                    new_lambda: ast.Lambda | None = self.visit_Lambda(stmt.value)
                    if new_lambda is not None:
                        new_body.append(ast.Expr(value=new_lambda))
                    else:
                        new_body.append(stmt)
                elif isinstance(stmt.value, ast.Call):
                    new_call: ast.Call = self.visit_Call(stmt.value)
    ...
    def _transform_aug_assign_in_function(self, node: ast.AugAssign) -> ast.AST | None:
        ...
        if name not in self.reactive_names:
            return None
        self.needs_var_import = True
        left: ast.Attribute = ast.Attribute(
            value=ast.Name(id=name, ctx=ast.Load()),
            attr='value',
            ctx=ast.Load(),
        )
        bin_op: ast.BinOp = ast.BinOp(
            left=left,
            op=node.op,
            right=node.value,
        )
...
class _ReactiveVarLoader(importlib.abc.Loader):
    ...
    def get_code(self, fullname: str) -> types.CodeType:
        source: str | None = self.get_source(fullname)
        if source is None:
            return self._original_loader.get_code(fullname)
        try:
            tree: ast.Module = ast.parse(source, filename=self._source_path)
            transformer = _ReactiveVarTransformer()
            transformed_tree: ast.Module = transformer.transform(tree, filename=self._source_path)
    ...
    def exec_module(self, module: Any) -> None:
        code: types.CodeType | None = self.get_code(module.__name__)
        if code is None:
...
class _ReactiveVarFinder:
    ...
    def find_spec(self, fullname: str, path: Any = None, target: Any = None) -> Any:
        ...
        if spec.origin:
            import sysconfig
            stdlib_dir: str | None = sysconfig.get_path('stdlib')
...
class VarNamespace:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if name.startswith('_'):
            ...
            return
        try:
            reactive_vars: dict[str, Var] = object.__getattribute__(self, '_vars')
