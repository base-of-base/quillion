...
class TwoWayBindingElement(Component, metaclass=TwoWayComponentMeta):
    ...
    def __init__(self, bind_var: Var | None = None, **kwargs: Any) -> None:
    ...
    def get_current_value(self) -> str:
        value: str = str(self._bind_var.value) if self._bind_var else ""
        return value
    ...
    def update_var_from_event(self, event_value: str) -> None:
        ...
        if isinstance(cur, int):
            event_value = int(event_value) if event_value else 0
        elif isinstance(cur, float):
            event_value = float(event_value) if event_value else 0.0
        elif isinstance(cur, bool):
            event_value = event_value.lower() in ("true", "1", "yes", "on")
